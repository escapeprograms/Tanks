...
io.on("connection", (socket)=>{
  ...
  //spawn a bot
  socket.on("bot",(obj)=>{
    console.log("spawned manually bot")
...
//Bot Names
var botNames = ["robot","bot","Botty McBotface","imbotster","robo cop","Marco"];

//Game
var fps = 40;//frames per sec client side
var tps = 40;//tick per sec server side
var lastUpdate = 0;//ms of last update
setInterval(()=>{
  //check all players
    ...
    for (var i = 0; i < fps/tps; i++){//loop to match tick per sec with fps
      ...
      if (p.shooting){
        p.guns.forEach((g)=>{
          ...
          bullets.push(new game.Bullet(p.id, g.pos[0],g.pos[1],Math.cos(angle) * 15, Math.sin(angle) * 15, p.color, game.tankTypes[p.type].dur, p.dmg));
        });
      ...
      if (p.hp <= 0){
        ...
        if (p.id != "") {
          for (var i = 0; i < players.length; i++){
            if (players[i].id == p.lastHit){
              ...
              killer.kills ++;
              killer.hp += killer.kills*10;//regenerate health
              if (killer.hp > game.tankTypes[killer.type].hp)
                killer.hp = game.tankTypes[killer.type].hp;
        ...
        if (p.hp <= -40) {
          for (var s = 0; s < 12; s++){
            bullets.push(new game.Bullet(p.id, p.pos[0],p.pos[1],Math.random()*50-25, Math.random()*50-25,p.color, Math.random()*10+20, 5));//shrapnel explosion
          }
      ...
      if (p.pos[0]*p.pos[0] + p.pos[1]*p.pos[1] > 2000**2) {
        p.hp -= 30;
      }
    }
  });
  ...
  //fill the lobby
  if (players.length < 3) {
    players.push(new game.Player("bot"+Math.random(), botNames[Math.floor(Math.random()*botNames.length)], Math.floor(Math.random()*game.tankTypes.length)));
  }
  
  //send all player data and bullet data
  io.emit("ping",[players,bullets]);
  //check server tps
  const d = new Date();
  var realTps = 1000/(d.getTime()-lastUpdate);
  io.emit("message2",`Server running at ${realTps} tps`)
